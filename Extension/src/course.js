...
function enhanceVideoMaterialDownloads() {
    ...
    videos.forEach((video, index) => {
        ...
        if (!sourceInfo.blobUrl && !sourceInfo.directUrl && !sourceInfo.originalUrl) {
            log('Skipping video download enhancement because no source URL was found');
            return;
        }

        video.dataset.uxVideoDownloadEnhanced = 'true';

        createVideoDownloadOptions(video, sourceInfo, contentName, videos.length > 1 ? (index + 1) : null);
    });
}
...
function getCurrentVideoPageNumber() {
    ...
    try {
        ...
        const activeButton = chapterDoc.querySelector('tr.bkkhaki input[name="clickpage"], td.bkkhaki input[name="clickpage"]');
        if (activeButton) {
...
function buildVideoMaterialBaseName(contentName, pageNumber, suffixIndex = null) {
...
function getVideoDownloadSourceInfo(video) {
    ...
    const directUrl = currentSrc && !currentSrc.startsWith('blob:') ? currentSrc : null;
    ...
    try {
        ...
        if (fileParam) {
            ...
            if (/^https?:/i.test(decodedFile)) {
                ...
                const contentsUrl = decodeURIComponent(urlObj.searchParams.get('contents_url') || '');
                if (contentsUrl) {
                    originalUrl = new URL(decodedFile, window.location.origin + contentsUrl).href;
                } else {
    ...
    const hlsUrl = isHlsPlaylistUrl(originalUrl)
        ? originalUrl
        : (isHlsPlaylistUrl(directUrl) ? directUrl : null);
    const originalExtension = getExtensionFromUrl(originalUrl || directUrl || '') || (hlsUrl ? '.m3u8' : null);
...
function getCurrentVideoDownloadBaseName(contentName, suffixIndex = null) {
    return sanitizeFileName(
        buildVideoMaterialBaseName(contentName, getCurrentVideoPageNumber(), suffixIndex)
    );
}
...
function getPreferredDirectVideoExtension(sourceUrl) {
    return normalizeVideoExtensionForCompatibility(getExtensionFromUrl(sourceUrl) || '.mp4');
}
...
function shouldRenameVideoResponseToMp4(contentType, originalExtension) {
    ...
    const normalizedExtension = normalizeVideoExtensionForCompatibility(originalExtension);
...
function createVideoDownloadOptions(video, sourceInfo, contentName, suffixIndex = null) {
    const container = document.createElement('div');
    container.className = 'ux-download-group';
    container.style.margin = '12px auto 0';
    container.style.maxWidth = `${Math.max(video.clientWidth || video.width || 480, 320)}px`;

    const initialBaseName = getCurrentVideoDownloadBaseName(contentName, suffixIndex);
    const estimatedFileName = getCompatibleVideoFileName(initialBaseName, sourceInfo);
    ...
    saveBtn.addEventListener('click', async (e) => {
        ...
        try {
            const baseName = getCurrentVideoDownloadBaseName(contentName, suffixIndex);
            if (sourceInfo.hlsUrl) {
                const fileName = await downloadHlsVideo(sourceInfo.hlsUrl, baseName, (completed, total) => {
                    saveBtn.textContent = total > 0 ? `取得中 ${completed}/${total}` : '取得中';
                });
...
async function openVideoFullscreenOverlay(video, contentName) {
    ...
    const frameEl = window.frameElement ||
        topDoc?.querySelector('frame[name="webclass_content"], iframe[name="webclass_content"]');
    ...
    if (frameEl) {
        ...
        const cleanupFullscreenLayout = video ? activateVideoFrameFullscreenLayout(video) : null;
        try {
...
function prepareOwningFrameForVideoFullscreen(frameEl) {
    ...
    if (!/\bfullscreen\b/i.test(allow)) {
        frameEl.setAttribute('allow', allow ? `${allow}; fullscreen *` : 'fullscreen *');
    }
}
...
function activateVideoFrameFullscreenLayout(video) {
    ...
    const layoutRoot = video.closest('.contentfile') || video.parentElement || video;
    ...
    return () => {
        document.documentElement.classList.remove('ux-video-frame-fullscreen-active');
        if (document.body) {
...
async function downloadRemoteFileAsBlob(url, filename) {
    ...
    const targetFilename = shouldRenameVideoResponseToMp4(blob.type, originalExtension)
...
function getHlsVariantScore(attributes) {
    ...
    const pixels = resolutionMatch
        ? ((parseInt(resolutionMatch[1], 10) || 0) * (parseInt(resolutionMatch[2], 10) || 0))
        : 0;
...
function parseHlsByteRange(value, previousByteRange = null) {
    ...
    const start = Number.isFinite(explicitStart)
        ? explicitStart
        : (previousByteRange ? previousByteRange.end + 1 : 0);
...
function buildHlsSegmentPlan(manifestText, playlistUrl) {
    ...
    for (const line of lines) {
        if (line.startsWith('#EXT-X-KEY:')) {
            const attributes = parseHlsAttributeList(line.slice('#EXT-X-KEY:'.length));
        ...
        const previousByteRange = previousSegmentReference?.url === resolvedUrl
            ? previousSegmentReference.byteRange
            : null;
        const byteRange = parseHlsByteRange(pendingSegmentByteRange, previousByteRange);
...
function needsStandaloneMp4Rebuild(mp4Data) {
    const normalized = mp4Data instanceof Uint8Array ? mp4Data : new Uint8Array(mp4Data);
...
async function ensureMp4BoxModule() {
    if (!mp4BoxModulePromise) {
        mp4BoxModulePromise = import(chrome.runtime.getURL('lib/mp4box.all.js'))
            .catch((error) => {
...
async function rebuildFragmentedMp4ToStandalone(mp4Data) {
    const normalizedInput = mp4Data instanceof Uint8Array ? mp4Data : new Uint8Array(mp4Data);
    if (!normalizedInput.byteLength || !needsStandaloneMp4Rebuild(normalizedInput)) {
    ...
    inputFile.onError = (...args) => {
        parseErrorMessage = args.filter((value) => typeof value === 'string' && value).join(': ');
    };
    ...
    for (const trak of inputFile.moov.traks) {
        for (let sampleIndex = 0; sampleIndex < trak.samples.length; sampleIndex += 1) {
            const sample = inputFile.getSample(trak, sampleIndex);
            if (!sample?.data) {
                throw new Error(`MP4 sample data is incomplete for track ${trak.tkhd.track_id}.`);
            }
        }
    }

    inputFile.boxes = inputFile.boxes.filter((box) => box.type !== 'moof' && box.type !== 'mdat');
    inputFile.moov.boxes = inputFile.moov.boxes.filter((box) => box && box.type !== 'mvex');
    ...
    for (const trak of inputFile.moov.traks) {
        ...
        if (!stbl || !stco || !stsc || !stsz || !stts) {
            throw new Error(`MP4 sample tables are unavailable for track ${trak?.tkhd?.track_id || 'unknown'}.`);
        }

        const samples = trak.samples || [];
        const trackChunkSize = samples.reduce((sum, sample) => sum + sample.size, 0);
        ...
        const durationRuns = runLengthEncode(samples.map((sample) => sample.duration));
        ...
        if (compositionOffsets.some((offset) => offset !== 0)) {
            if (!stbl.ctts && MP4Box?.BoxParser?.box?.ctts) {
                ...
                stbl.ctts = ctts;
                const stssIndex = stbl.boxes.findIndex((box) => box && box.type === 'stss');
                const stscIndex = stbl.boxes.findIndex((box) => box && box.type === 'stsc');
                const insertIndex = stssIndex >= 0 ? stssIndex + 1 : (stscIndex >= 0 ? stscIndex : stbl.boxes.length);
        ...
        const trackMovieDuration = trak.mdia.mdhd.timescale
            ? Math.round((trak.samples_duration * inputFile.moov.mvhd.timescale) / trak.mdia.mdhd.timescale)
            : 0;
        trak.tkhd.duration = trackMovieDuration;
        movieDuration = Math.max(movieDuration, trackMovieDuration);

        for (const sample of samples) {
            mdatChunks.push(sample.data instanceof Uint8Array ? sample.data : new Uint8Array(sample.data));
        }
    }
    ...
    for (let trackIndex = 0; trackIndex < inputFile.moov.traks.length; trackIndex += 1) {
...
async function normalizeMp4ForCompatibleDownload(mp4Data) {
    ...
    try {
        ...
        console.warn('[WebClass UX] Failed to rebuild fragmented MP4 into standalone MP4:', error);
        return normalizedInput;
    }
}
    ...
    return new Promise((resolve, reject) => {
        ...
        try {
            for (const segmentBuffer of segmentBuffers) {
                const chunk = segmentBuffer instanceof Uint8Array
                    ? segmentBuffer
...
async function transmuxTsSegmentsToMp4(segmentBuffers) {
    ...
    const normalizedBuffers = segmentBuffers.map((segmentBuffer) =>
        segmentBuffer instanceof Uint8Array ? segmentBuffer : new Uint8Array(segmentBuffer)
    );
    const attempts = [
        {
    ...
    for (const attempt of attempts) {
        try {
            return await transmuxTsSegmentsToMp4Once(attempt.buffers, attempt.options);
        } catch (error) {
            lastError = error;
            console.warn('[WebClass UX] MP4 transmux attempt failed:', attempt.name, error);
        }
    }
...
async function downloadHlsVideo(hlsUrl, baseName, onProgress) {
    ...
    const { initSegmentUrl, segmentUrls } = buildHlsSegmentPlan(manifestText, playlistUrl);
    ...
    if (initSegmentUrl) {
        const initPart = await fetchArrayBufferWithCredentials(initSegmentUrl.url, initSegmentUrl.byteRange);
        parts.push(initPart.buffer);
        detectedFormat = detectHlsOutputFormat(initSegmentUrl.url, initPart.contentType, true);
        completed += 1;
        if (typeof onProgress === 'function') {
            onProgress(completed, total);
        }
    ...
    for (const segmentUrl of segmentUrls) {
        const segment = await fetchArrayBufferWithCredentials(segmentUrl.url, segmentUrl.byteRange);
        if (!detectedFormat) {
            detectedFormat = detectHlsOutputFormat(segmentUrl.url, segment.contentType, false);
        }
    ...
    if (finalFormat.extension === '.ts') {
        try {
            ...
            console.warn('[WebClass UX] Falling back to TS download after MP4 transmux failure:', error);
            const fileName = `${baseName}.ts`;
            await downloadBlobData(
                new Blob(parts, { type: 'video/mp2t' }),
                fileName
            );
            return fileName;
        }
    ...
    if (finalFormat.extension === '.mp4') {
        const mp4Data = await normalizeMp4ForCompatibleDownload(
            concatUint8Arrays(parts.map((part) =>
...
function enhanceCourseContentsPageUI() {
    ...
    const ensureQuickNavShell = () => {
        ...
        const setCollapsedState = (collapsed) => {
            ...
            document.body.style.setProperty('--ux-course-quick-nav-width', collapsed ? '64px' : '280px');
            if (toggleButton) {
